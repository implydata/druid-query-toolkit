...
describe('Druid Query Tests', () => {
  ...
  it('parsers the default data sources query to string with spaces', () => {
    expect(
      ...
      SELECT
        datasource,
        COUNT(* )undefinedFILTER (WHERE (is_published = 1 AND is_overshadowed = 0) OR is_realtime = 1) AS num_segments,
        COUNT(* )undefinedFILTER (WHERE is_available = 1 AND ((is_published = 1 AND is_overshadowed = 0) OR is_realtime = 1)) AS num_available_segments,
        COUNT(* )undefinedFILTER (WHERE is_published = 1 AND is_overshadowed = 0 AND is_available = 0) AS num_segments_to_load,
        COUNT(* )undefinedFILTER (WHERE is_available = 1 AND NOT ((is_published = 1 AND is_overshadowed = 0) OR is_realtime = 1)) AS num_segments_to_drop,
        SUM(\\"size\\" )undefinedFILTER (WHERE (is_published = 1 AND is_overshadowed = 0) OR is_realtime = 1) AS size,
        SUM(\\"size\\" * \\"num_replicas\\" )undefinedFILTER (WHERE (is_published = 1 AND is_overshadowed = 0) OR is_realtime = 1) AS replicated_size,
        SUM(\\"num_rows\\" )undefinedFILTER (WHERE (is_published = 1 AND is_overshadowed = 0) OR is_realtime = 1) AS num_rows
      FROM sys.segments
      GROUP BY 1 
      ;"
    `);
