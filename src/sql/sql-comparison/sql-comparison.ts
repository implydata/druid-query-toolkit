...
import { SqlBase, SqlBaseValue, SqlExpression, SqlLiteral, SqlRef } from '..';
...
export class SqlComparison extends SqlExpression {
  ...
  public containsColumn(column: string): boolean {
    ...
    if (SqlRef.equalsString(lhs, column) || lhs.containsColumn(column)) return true;

    if (
      rhs instanceof SqlExpression &&
      (SqlRef.equalsString(rhs, column) || rhs.containsColumn(column))
    ) {
      return true;
    }
